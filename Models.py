...
class Character(SQLModel, table=True):
    ...
    height: float = Field(nullable=True)
    weight: float = Field(nullable=True)
    hair_color: str = Field(nullable=True, max_length=30)
    eye_color: str = Field(nullable=True, max_length=30)
    blood_type: int = Field(nullable=False, foreign_key='blood_type.id')
    birth_place: int = Field(nullable=False, foreign_key='place.id')
    img_face: str = Field(nullable=True, max_length=250)
    img_body: str = Field(nullable=True, max_length=250)
